...
//	if(aRxBuffer[4] != '\0' ){
//		if(overFlow[0]='\0'){
//			overFlow[f] = aRxBuffer[4];
//		}else{
//
//			while(overFlow[f] != '\0' ){
//				overFlow[f+1] = aRxBuffer[4];
//				f++;
//			}
//		}
//
...
//	}
	...
	manualMode = 0;
	indval = val;
	if(aRxBuffer[0] == 'R' && aRxBuffer[1] == 'S' && aRxBuffer[2] == '0' && aRxBuffer[3] == '0'){
		__ACK_TASK_DONE2(&huart3, rxMsg);
		ttt = 'Y';
	}else if (aRxBuffer[0] == 'S' && aRxBuffer[1] == 'T') { // only STOP can preempt any greedy task
//		__ADD_COMMAND(cQueue, 0, 0); // stop
	...
	else if (aRxBuffer[0] == 'O' && aRxBuffer[1] == 'B') {
		step = 0;
		__ADD_COMMAND(cQueue, 16, val); // WX fastest path OB

	}
	else if (aRxBuffer[0] == 'U' && aRxBuffer[1] == 'L'){
		step = -1;
		__ADD_COMMAND(cQueue, 16, val); // WX fastest path OB ul00
	}
	else if (aRxBuffer[0] == 'U' && aRxBuffer[1] == 'R'){
		step = -2;
		__ADD_COMMAND(cQueue, 16, val); // WX fastest path OB ur00
	}	else if (aRxBuffer[0] == 'P' && aRxBuffer[1] == 'L'){
		step = -3;
		__ADD_COMMAND(cQueue, 16, val); // WX fastest path OB pl
	}	else if (aRxBuffer[0] == 'P' && aRxBuffer[1] == 'R'){
		step = -4;
		__ADD_COMMAND(cQueue, 16, val); // WX fastest path OB ur00
	}
...
//			  og_dist = (HAL_GetTick() - last_curTask_tick )* 0.0344 /2;
//			  og_dist = 1;
...
//			  og_dist = 2;
	...
	__SET_MOTOR_DUTY(&htim8, 0, 0);
	duration = HAL_GetTick() - timeStart;
...
//	OLED_ShowString(0, 0, (char *) ch);
	...
	sprintf(message1, "buff: %s\0", aRxBuffer);
	OLED_ShowString(0,30, message1);
	sprintf(message2, "fb: %d\0", frontBack);
	OLED_ShowString(0,40, message2);
//	sprintf(message3, "test: %d\0", ind);
	sprintf(message3, "d: %d\0", duration);
	OLED_ShowString(0,50, message3);
//	sprintf(message4, "val: %d\0", indval);
	sprintf(message4, "T: %d\0", totalDuration);
	OLED_ShowString(70,50, message4);
...
//	int dirArray[21] = {0, 6 , 7, 1};
//	int i = 0;
  /* Infinite loop */
  for(;;)
  {
//	  step = seqq;
//	  stepArray = dirArray[i];
//	  step = dirArray[i];
...
//			  __SET_MOTOR_DUTY(&htim8, 600, 1000);
			  __SET_MOTOR_DUTY(&htim8,1800, 3000);
			  __SET_MOTOR_DIRECTION(1);
			  HAL_TIM_IC_Start_IT(&htim4, TIM_CHANNEL_2);
			  last_curTask_tick = HAL_GetTick();
			  do {
...
//					  __Gyro_Read_Z(&hi2c1, readGyroZData, gyroZ);
					  ...
					  if (abs(targetAngle - angleNow) < 0.01) {
						  step = 2;
						  break;
					  }
					  last_curTask_tick = HAL_GetTick();
...
//			  HAL_Init();
			  ...
			  osDelay(150);
			  // move str8
			  __SET_MOTOR_DUTY(&htim8, 4000, 4000);
			  __RESET_SERVO_TURN_FAST(&htim1);
			  osDelay(100);

			  // turn right to hit centerline
			  __SET_MOTOR_DUTY(&htim8, 3050, 1400);
			  __SET_SERVO_TURN_MAX(&htim1, 1);
			  osDelay(900);
			  //turn left to align center
			  __SET_MOTOR_DUTY(&htim8, 1400, 3300);
			  __SET_SERVO_TURN_MAX(&htim1, 0);
			  osDelay(420);
			  //move str8
...
//			  step = 2;
			  ...
			  //     __SET_MOTOR_DUTY(&htim8, 1000, 600);
			       //turn right

				  float timeStart = 0.00;
			       osDelay(1000);
			       __SET_MOTOR_DIRECTION(1);
			       __SET_MOTOR_DUTY(&htim8,3000, 1800);
			       __SET_SERVO_TURN_MAX(&htim1, 1);
			       osDelay(200);
			       // move str8
			       __SET_MOTOR_DUTY(&htim8, 4000, 4000);
			       ...
			       // turn left to hit centerline
			       __SET_MOTOR_DUTY(&htim8,1400, 3050 );
			       __SET_SERVO_TURN_MAX(&htim1, 0);
			       osDelay(800);

			       //turn right to align center
			       __SET_MOTOR_DUTY(&htim8, 4000, 1300);
			       __SET_SERVO_TURN_MAX(&htim1, 1);
			       osDelay(400);
			       //move str8
			       __SET_MOTOR_DUTY(&htim8, 2000, 2000);
...
//			       osDelay(15000);
			  ...
			  //     RobotMoveDistObstacle(&targetDist, SPEED_MODE_2);
			       ...
			       __SET_MOTOR_DUTY(&htim8,2000, 2000);
			       osDelay(300);
			       //turn left 1m turning
			       __SET_MOTOR_DIRECTION(1);
			       __SET_MOTOR_DUTY(&htim8,1800, 3550);
			       __SET_SERVO_TURN_MAX(&htim1, 0);
			       osDelay(700);
			       ...
			       // turn right to go horizontal
			       __SET_MOTOR_DUTY(&htim8, 3500, 1800);
			       __SET_SERVO_TURN_MAX(&htim1, 1);
			       osDelay(2100);
			       //1770
			       //move str8 along ob2
			       __SET_MOTOR_DUTY(&htim8, 5000, 5000);
			       __RESET_SERVO_TURN_FAST(&htim1);
			       osDelay(1100);

				  if(totalDuration<2700 +900){
					  // turn r to move vertical
					  __SET_MOTOR_DUTY(&htim8, 3300, 1800);
					  __SET_SERVO_TURN_MAX(&htim1, 1);
					  osDelay(800);
					  //move st8 vertically
					  __SET_MOTOR_DUTY(&htim8, 5000, 5000);
					  __RESET_SERVO_TURN_FAST(&htim1);
					  osDelay(duration -100);
					  // turn r to move str8
					  __SET_MOTOR_DUTY(&htim8, 3500, 3500);
					  __SET_SERVO_TURN_MAX(&htim1, 1);
					  osDelay(500);
					  //move str8
					  __SET_MOTOR_DUTY(&htim8, 5000, 5000);
					  ...
					  //turn l to align center
					  __SET_MOTOR_DUTY(&htim8, 1800, 3500);
					  __SET_SERVO_TURN_MAX(&htim1, 0);
					  osDelay(400);
					  __SET_MOTOR_DUTY(&htim8, 3000, 3000);
					  __RESET_SERVO_TURN_FAST(&htim1);
					  osDelay(100);
					  targetDist = 20;
					  RobotMoveDistObstacle(&targetDist, SPEED_MODE_2);
	//				  __SET_MOTOR_DUTY(&htim8, 0, 0);
				  ...
				  else if(totalDuration<3300 +900){
					  // turn r to move vertical
					  __SET_MOTOR_DUTY(&htim8, 3300, 1800);
					  __SET_SERVO_TURN_MAX(&htim1, 1);
					  osDelay(700);
...
//			  RobotMoveDistObstacle(&targetDist, SPEED_MODE_2);
			  ...
			  __SET_MOTOR_DUTY(&htim8,2000, 2000);
			  osDelay(300);
			  //turn right 1m turning
			  __SET_MOTOR_DIRECTION(1);
			  __SET_MOTOR_DUTY(&htim8,3550, 1800);
			  __SET_SERVO_TURN_MAX(&htim1, 1);
			  osDelay(900);
			  ...
			  __RESET_SERVO_TURN_FAST(&htim1);
			  osDelay(1150);
			  timeStr8 = totalDuration;
			  if(totalDuration<2700+ 900){
				  // turn left to move vertical
				  __SET_MOTOR_DUTY(&htim8, 1800, 3500);
				  __SET_SERVO_TURN_MAX(&htim1, 0);
				  osDelay(750);
				  //move st8 vertically
