...
class RaspberryPi:
    ...
    def recv_stm(self) -> None:
        ...
        """
        while True:
            self.logger.debug("1111111111111111111 Recv_stm 111111111111111")
            message: str = self.stm_link.recv()
            # Added code for testing purpose
            # time.sleep(4)
            self.logger.debug("Timothy asked me to put this")
            self.logger.debug(message)
            # ended 
            self.logger.debug("22222222222222 Recv_Stm message 22222222222222")
            self.logger.debug(message)
            # if message.startswith("ACK"): # Change if ACK Check
    ...
    def android_sender(self) -> None:
        ...
        while True:
            # Retrieve from queue
            try:
                message: AndroidMessage = self.android_queue.get(timeout=2)
            except queue.Empty:
                continue
    ...
    def command_follower(self) -> None:
        ...
        """
        # ADDED COUNT = 0 #########################################
        self.logger.debug("In Command__follwer!!!!!!!!!!!!!!")
        count = 0
        self.logger.debug(count)
        # self.command_queue.get()
        while True:
            """ commented out the bottom chuck for testing_1
            # Retrieve next movement command
            self.logger.debug("11111111111111 before command get 11111111111111111111")
            self.logger.debug(count)
            command: str = self.command_queue.get()
            # ADDED ###################################################
            self.logger.debug("1111111111111 command::::::::::::22222222222")
            self.logger.debug(command)
            self.logger.debug(count)
            count +=1
            self.logger.debug("_2222222222 end of 2222222222222222")
            # ADD END ################################################
            ...
            try:
                ...
                # command: str = self.command_queue.get() # added this line for testing_1
                self.retrylock.acquire() # Commented out this line for testing
                # command: str = self.command_queue.get() # added this line for testing_2
                self.retrylock.release() # Commented out this line for testing
                # command: str = self.command_queue.get() # added this line for testing_1
                # self.logger.debug(command)
            except:
                self.logger.debug("wait for unpause")
            ...
            self.logger.debug("33333333 Command after aquire 3333333333")
            # self.logger.debug(command)
            # STM32 Commands - Send straight to STM32
            stm32_prefixes = ("FS", "BS", "FW", "BW", "FL", "FR", "BL",
                              "BR", "TL", "TR", "A", "C", "DT", "STOP", "ZZ", "RS")
            self.logger.debug("5555555 b4 command.start 55555555555")
            command: str = self.command_queue.get()
            if command.startswith(stm32_prefixes):
            #if self.command_queue.get().startswith(stm32_prefixes):
                ######################################################ADDED
                self.logger.debug("44444444444 command in startprefix 444444444444444444")
                self.logger.debug(command)
    ...
    def snap_and_rec(self, obstacle_id_with_signal: str) -> None:
        ...
        self.logger.debug("testing testing 123123123123123123123123123123123123123123")
        self.logger.debug(os.path.exists('/home/pi/mdp/rpi/images'))
        filename = f"/home/pi/mdp/rpi/images/{int(time.time())}_{obstacle_id}_{signal}.jpg"
        ...
        while True:
            ...
            if (shutter * 1000000) - int(shutter * 1000000) > 0.5:
                sspeed += 1
            # made changes from libcamera to raspistill
            """
            ...
            """
            rpistr = "libcamera-still -o " + filename
            self.logger.debug(rpistr)
            os.system(rpistr)
        ...
        self.logger.debug("))))))))))))))))))))))))))))))))))))))))))))))))")
        self.logger.debug(AndroidMessage("image-rec",results))
    ...
    def request_algo(self, data, robot_x=1, robot_y=1, robot_dir=0, retrying=False):
        ...
        # Added this code for error checking
        self.logger.debug("########################################")
